...
import { useYieldRates } from './useYieldRates'
...
export const useYieldDisplayValues = ({
  apiClient,
}: UseSwapDisplayValuesParams): SwapDisplayValues => {
  const sellAsset = YieldMachineCtx.useSelector(s => s.context.sellAsset)
  const buyAsset = YieldMachineCtx.useSelector(s => s.context.buyAsset)
  const sellAmountBaseUnit = YieldMachineCtx.useSelector(s => s.context.sellAmountBaseUnit)
  const isSellAssetEvm = YieldMachineCtx.useSelector(s => s.context.isSellAssetEvm)
  const isSellAssetUtxo = YieldMachineCtx.useSelector(s => s.context.isSellAssetUtxo)
  const isSellAssetSolana = YieldMachineCtx.useSelector(s => s.context.isSellAssetSolana)
  const selectedRate = YieldMachineCtx.useSelector(s => s.context.selectedRate)

  const { walletAddress, effectiveReceiveAddress, bitcoin, solana } = useYieldWallet()
