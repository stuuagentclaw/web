...
import type { YieldWalletContextValue } from '../contexts/YieldWalletContext'
...
const YieldWidgetContent = ({
  ...
  sellAllowedAssetIds,
  buyAllowedAssetIds,
}: YieldWidgetContentProps) => {
  const state = YieldMachineCtx.useSelector(s => s)
  ...
  const {
    ...
    buyBalanceFiatValue,
  } = useYieldDisplayValues({ apiClient })
  ...
  const {
    ...
    handleButtonClick,
  } = useYieldHandlers({ onConnectWallet, onAssetSelect, partnerCode, appUrl })

  useYieldQuoting({ apiClient, rates, sellAssetBalance })
...
const YieldWidgetCore = ({
  ...
  const buyChainId = YieldMachineCtx.useSelector(s => s.context.buyAsset.chainId)
  ...
  const walletValue: YieldWalletContextValue = useMemo(
    () => ({
      walletClient,
      walletAddress,
      effectiveReceiveAddress,
  ...
  return (
    <YieldWalletProvider value={walletValue}>
      <YieldWidgetContent
        apiClient={apiClient}
        theme={theme}
...
const YieldWidgetWithExternalWallet = (props: YieldWidgetProps) => {
...
const YieldWidgetWithInternalWallet = (
  props: YieldWidgetProps & { walletConnectProjectId: string },
) => {
  const apiClient = useMemo(
    () =>
      createApiClient({
...
export const YieldWidget = (props: YieldWidgetProps) => {
  if (props.enableWalletConnection && props.walletConnectProjectId) {
    return (
      <YieldWidgetWithInternalWallet
        {...props}
  ...
  return <YieldWidgetWithExternalWallet {...props} />
}
